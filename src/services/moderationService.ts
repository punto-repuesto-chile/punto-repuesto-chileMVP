...
import type {
  ...
  RestoreReportTargetInput,
  UpdateModerationReportStatusInput,
} from "../types/moderation"

type ModerationErrorKind = "forbidden" | "conflict" | "not_found" | "invalid_transition" | "content_already_hidden" | "content_not_hidden" | "content_deleted" | "content_unavailable" | "invalid_target" | "invalid_target_part" | "stale_state" | "report_status_conflict" | "moderation_hold_conflict" | "unknown"
...
type ActionRow = {
  ...
  created_at: string
  content_moderation_id: string | null
  previous_content_state: ContentModerationState | null
  new_content_state: ContentModerationState | null
}
...
type ReportDetailRow = Omit<ReportListRow, "has_details" | "snapshot_summary"> & {
  ...
  moderation_origin_report_id: string | null
  moderation_hidden_at: string | null
  moderation_hidden_by_name: string | null
  moderation_id: string | null
}

type ContentModerationUpdateRow = {
  report_id: string
  report_status: ModerationReportStatus
  moderation_id: string
  moderation_state: "hidden" | "restored"
  moderation_version: number
}
...
function throwModerationError(error: RpcError): never {
  ...
  const stableErrors: Partial<Record<ModerationErrorKind, string>> = {
    content_already_hidden: "Este contenido ya está oculto por moderación.",
    content_not_hidden: "Este contenido ya no está oculto.",
    content_deleted: "El contenido fue eliminado y no se puede ocultar.",
    content_unavailable: "El contenido no está disponible para esta acción.",
    invalid_target: "El objetivo del reporte no es válido.",
    invalid_target_part: "La parte reportada del contenido no es válida.",
    stale_state: "El estado de moderación cambió. Actualiza el reporte.",
    report_status_conflict: "Este reporte fue actualizado por otro moderador.",
    moderation_hold_conflict:
      "Otro moderador cambió este contenido. Actualiza el reporte.",
  }

  const stableKind = Object.keys(stableErrors).find((kind) =>
    detail.includes(kind),
  ) as ModerationErrorKind | undefined
  if (stableKind)
    throw new ModerationServiceError(
      stableErrors[stableKind]!,
      stableKind,
      error.code,
    )
...
function mapAction(row: ActionRow): ModerationAction {
  return {
    ...
    contentModerationId: row.content_moderation_id,

    previousContentState: row.previous_content_state,

    newContentState: row.new_content_state,
  }
}
...
export async function getReport(
  ...
  return {
    ...
    moderationOriginReportId: row.moderation_origin_report_id,
    moderationHiddenAt: row.moderation_hidden_at,
    moderationHiddenByName: row.moderation_hidden_by_name,
    moderationId: row.moderation_id,
  }
}

function mapContentModerationUpdate(
  row: ContentModerationUpdateRow,
): ContentModerationUpdate {
  return {
...
export async function hideReportTarget(
  input: HideReportTargetInput,
): Promise<ContentModerationUpdate> {
  const { data, error } = await supabase
    .rpc("admin_hide_report_target", {
      p_report_id: input.reportId,
      p_expected_report_status: input.expectedReportStatus,
      p_expected_moderation_version: input.expectedModerationVersion ?? 0,
      p_note: input.note?.trim() || null,
    })
    .single()
  if (error) throwModerationError(error)
  return mapContentModerationUpdate(data as ContentModerationUpdateRow)
}

export async function restoreReportTarget(
  input: RestoreReportTargetInput,
): Promise<ContentModerationUpdate> {
  const { data, error } = await supabase
    .rpc("admin_restore_report_target", {
      p_report_id: input.reportId,
